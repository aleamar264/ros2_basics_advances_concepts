#include "challenge_actions_interfaces/action/RobotPosition.hpp"
...
using MoveRobot = challenge_actions_interfaces::action::RobotPosition;
...
public:
    ...
    LifecycleCallbackReturn on_configure(
        ...
        move_robot_server_ = rclcpp_action::create_server<MoveRobot>(
            this, "move_robot_" + robot_name_,
            std::bind(&MoveRobotServerNode::goal_callback, this, _1, _ 2),
            std::bind(&MoveRobotServerNode::cancel_callback, this, _1),
            std::bind(&MoveRobotServerNode::handle_accepted_callback, this, _1),
            rclcpp_acton_server_get_default_options(), cb_group_);
...
private:
    rclcpp_action::GoalResponse goal_callback(
        cosnt rclcpp_action::GoalUUID &uuid,
        std::shared_ptr<const MoveRobot::Goal> goal) {
        (void)uuid;
        RCLCPP_INFO(this->get_logger(), "Received a new goal");
        // Check if node is active
        if (!server_activated_) {
            RCLCPP_INFO(this->get_logger(), "Node not activated yet");
        ...
        if ((goal->position < 0) || (goal->position > 100) ||
            ...
            RCLCPP_INFO(this->get_logger(),
                        "Invalid position/velocity, reject goal");
            return rclcpp_action::GoalResponse::REJECT;
        }
    ...
    void handle_accepted_callback(
        onst std::shared_ptr<MoveRobotGoalHandle> goal_handle) {
        execute_goal(goal_handle);
    }

    void execute_goal(onst std::shared_ptr<MoveRobotGoalHandle> goal_handle) {
        {
            std::lock_guard<std::mutex> lock(mutex_);
        ...
        while (rclcpp::ok()) {
            {
                ...
                if (goal_handle->get_goal_id() == preempted_goal_id_) {
                    ...
                    result->message =
                        "Preempted by another goal, or deactivate";
            ...
            RCLCPP_INFO(this->get_logger(), "Robot position %d",
                        robot_position_)
